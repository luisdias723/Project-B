<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="Marca" sortable min-width="170">
        <template #default="scope">
      ...
      >
        <template #default="scope">
          <router-link :to="'/truckBrand/' + scope.row.id">
    ...
    >
      <div v-loading="TruckBrandCreating" class="form-container">
        <el-form
          ref="TruckBrandForm"
          :rules="rules"
          :model="newBrand"
        ...
        >
          <el-form-item label="Marca" prop="brand">
            <el-input v-model="newBrand.brand" />
          </el-form-item>
        </el-form>
      </div>
      <template #footer>
        ...
        <el-button round type="primary" @click="createBrand()">
          {{ $t("Confirmar") }}
        </el-button>
      </template>
    </el-dialog>
...
const truckBrandResource = new Resource("truckBrand");
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        brand: [
          { required: true, message: "Escolha uma marca", trigger: "blur" },
        ],
      },
    };
  },
  ...
  methods: {
    async getList(sort) {
      ...
      this.loading = true;
      const { data, meta } = await truckBrandResource.list(this.query);
      this.list = data;
    ...
    handleCreate() {
      ...
      this.$nextTick(() => {
        this.$refs["TruckBrandForm"].clearValidate();
      });
    },
    ...
    createBrand() {
      this.$refs["TruckBrandForm"].validate((valid) => {
        if (valid) {
          // this.newTruckroles = [this.newTruck.role];
          this.TruckBrandCreating = true;
          truckBrandResource
            .store(this.newBrand)
            .then((response) => {
              this.$message({
                message:
                  "Marca (" + this.newBrand.brand + ") criada com sucesso.",
                type: "success",
                duration: 5 * 1000,
              });
              this.resetNewBrand();
